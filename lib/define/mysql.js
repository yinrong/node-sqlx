...
MysqlConnection.prototype.delete = function(table, where, callback) {
  ...
  function(rows, _, next) {
    if (rows.length === 0) {
      return callback(null, {affected_rows: 0}, null)
    }
    old_rows = rows
  ...
  function(result, _, next) {
    ...
    result = local.convertCamelCaseToSnakeCase(result)
  ...
  if (sets.constructor === Array) {
    ...
    async.eachSeries(sets, function(item, next) {
      that.insert(table, item, function(err, rows, info){
        ...
        ids.push(rows.insert_id)
        next()
      })
    }, function(err) {
      callback(err, {insert_ids: ids}, null)
    })
    return
  }
  if (!sets || sets.constructor !== Object
...
local.secureObjectValues = (o, attr) => {
  if (o === undefined || o === null) {
    return attr ? true : false
  }
  if (o.constructor === Object) {
    for (var key in o) {
      if (local.secureObjectValues(o[key], true) === false) {
        return false
      }
    }
  }
  if (o.constructor === Array) {
    o.forEach(item => {
      if (!local.secureObjectValues(item, true)) {
        return false
      }
    })
  }
  if (o.constructor === String) {
    for (let i = 0; i < UNSAFE_STRING.length; i++) {
      if (o.toString().toLowerCase().indexOf(UNSAFE_STRING[i]) > -1) {
        // ignore dangerous characters
        return false
      }
    }
  }
...
local.secureCheckResult = (o) => {
  let result = local.secureObjectValues(o, false)
  return result ? o : false
}

local.convertCamelCaseToSnakeCase = (object) => {
  let arr = Object.keys(object)
  let result = {}
  arr.forEach(item => {
    let s = item.replace(/([A-Z])/g, '_$1').toLowerCase()
    result[s] = object[item]
  })
  return result
}
